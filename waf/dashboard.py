...
def generate_figure(df):
	...
	fig.add_trace(go.Pie(labels=data_pie['type'], values=data_pie['count'], title='Performed requests', textposition='inside', textinfo='percent_l'))
...
@app.callback([Output('table-data', 'children'), Output('graph', 'children')], [Input('example-graph1', 'clickData'), Input('reset-button', 'n_clicks')])
...
def review_request(request_id):
	with open('./requests_log/' + str(request_id) + '.json', 'r') as f:
		request = json.dump(json.load(f), indent=4)
